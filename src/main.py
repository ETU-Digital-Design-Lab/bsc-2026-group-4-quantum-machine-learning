...
def get_device(wires):
    """
    Windows ortamında sorunsuz kurulan ve Qiskit'ten katbekat 
    daha hızlı çalışan C++ tabanlı PennyLane CPU simülatörü.
    """
...
# ==========================================
def load_data(name, sample_limit=150):
    if name == "Iris": data = load_iris()
    elif name == "Wine": data = load_wine()
    else: data = load_breast_cancer()

    X, y = data.data, data.target
    # Çok sınıflı veri setlerini (Iris, Wine) ikili (binary) sınıfa dönüştürür
    if len(np.unique(y)) > 2: y = (y == np.unique(y)[0]).astype(int)
    
    # Hızlı analiz için örneklem limiti uygular (istendiğinde artırılabilir)
...
def prepare_features(X_tr, X_te, enc_type, q):
    ...
    # PennyLane'in tensörlerine dönüştür (Gradyan takibi için gerekli format)
...
# --- QSVM Çekirdek Devresi ---
def qsvm_kernel(X_train, X_test, enc_type, q):
    dev = get_device(q)
    
    # QSVM gradyan (türev) kullanmaz, bu yüzden diff_method parametresi YOKTUR!
    @qml.qnode(dev)
    ...
    # Kuantum Çekirdek Matrisi (Kernel Matrix) Hesaplamaları
...
def run_vqc(X_train, y_train, X_test, enc_type, q, iters=10):
    ...
    # VQC optimizasyon (öğrenme) için türev kullanır, diff_method="adjoint" ZORUNLUDUR!
    ...
    # Etiketleri (0, 1) aralığından kuantum ölçüm aralığına (-1, 1) çek
    y_tr_vqc = np.where(y_train == 0, -1, 1)
    
    # Maliyet (Loss) fonksiyonu (Hata Kareler Ortalaması)
    ...
    # Kuantum Devresini Eğitme (Mini-batch Gradient Descent)
...
def execute_benchmark():
    ...
    print("=" * 80)
    print("🚀 C++ HIZLANDIRMALI QML & CML BENCHMARK MOTORU BAŞLADI")
    print("Cihaz: PennyLane (lightning.qubit) CPU Hızlandırıcı Aktif")
    print("=" * 80)

    for ds in datasets:
        print(f"\n🔄 Veri Seti İşleniyor: {ds}")
        X, y = load_data(ds)
        X = StandardScaler().fit_transform(X)
        
        for val_method in validation_methods:
            splits = []
            # Çapraz Doğrulama Türüne Göre Veri Bölme
            ...
            for q in qubits_list:
                for enc in encodings:
                    for m in models:
                        # Klasik SVM için gereksiz kuantum parametre döngülerini atla
                        ...
                        results.append({
                            "Dataset": ds, "Validation": val_method, "Model": m,
                            "Encoding": enc_label, "Qubits": q_label, 
                            "Accuracy": avg_acc, "Time(s)": avg_time
                        })

    df = pd.DataFrame(results)
    df.to_csv("qml_benchmark_lightning.csv", index=False)
    analyze_results(df)
...
def analyze_results(df):
    print("\n" + "=" * 80)
    print("📊 PERFORMANS VE ETKİ ANALİZ RAPORU")
    print("=" * 80)
    ...
    # Klasik modelleri analiz dışı bırakıyoruz ki encoding karşılaştırması doğru olsun
